...
/**
 * Service for decoding official WPILib binary `.wpilog` data trace files.
 *
 * Implements high-throughput binary stream decoding for WPILib log files generated by DataLogManager or AdvantageKit.
 * Parses entry declaration control records and decodes bit-packed primitive and array payloads.
 *
 * ### Binary File Layout & Protocol Specifications:
 * - Magic Header: 12 bytes total starting with `"WPILOG"` ASCII magic string
 * - Variable-length integer headers encoding `entryId`, `payloadSize`, and `timestampMicro`
 * - Timestamp conversion: $t_{\text{ms}} = \frac{t_{\mu \text{s}}}{1000}$
 * - Control Record 0 (Start): Entry ID mapping to topic key and data type string (`"double"`, `"boolean"`, `"string"`, `"double[]"`)
 * - Data Records: Typed value updates pushed to [FrameBatcher]
 *
 * ### Thread Safety & Performance Guarantees:
 * Executes asynchronously on `Dispatchers.IO`. Streams binary file sequential bytes via [FileInputStream] into [FrameBatcher].
 *
 * @see BaseLogDecoder
...
class WpiLogDecoder {
    ...
    /**
     * Parses a WPILib `.wpilog` file into normalized [TelemetryFrame] channels.
     *
     * @param file Source `.wpilog` binary file.
     * @param sessionId Session identifier string.
     * @param batcher Destination telemetry frame batch channel.
     */
