...
object LearningCatalog {
    ...
    val lessons: List<LearningLesson> = listOf(
        ...
        LearningLesson(
            ...
            steps = listOf(
                "Create a coordinator and explain why the mechanisms must move together.",
                "Add the same generated target fields to every complete posture.",
                "Choose the startup and fault-neutral postures, then verify the fault values are locked to subsystem neutral.",
                "Add action, fresh-sensor, or elapsed-time transitions from project-backed choices.",
                "Add only the interlocks and lookup tables that have a stated unit and safety purpose.",
                "Review the project errors, exact destination, content hash, and generated runtime guarantees before saving.",
            ),
            successLooksLike = "Every posture commands the same fields, every non-fault posture is reachable, the fault posture is neutral, and the reviewed canonical document passes project validation.",
            safetyNote = "The Studio writes project files only. Simulation does not validate physical clearance, wiring, sensor polarity, or current limits.",
            keywords = setOf("superstructure", "coordinator", "posture", "state machine", "interlock", "lookup table", "lut"),
            prerequisiteLessonIds = setOf("safe-subsystem"),
            checkpoints = listOf(
                reflectionCheckpoint(
                    "coordinate-mechanisms.postures",
